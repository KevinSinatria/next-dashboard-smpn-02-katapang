...
export function ArticleCategoriesDataTable() {
  ...
  const { articleCategories, meta, isLoading, mutate } = useArticleCategories({
    page,
  ...
  useEffect(() => {
    if (isLoading) {
      toast.loading("Memuat data...", { id: "article-categories" });
    } else {
      toast.dismiss("article-categories");
    }
  }, [isLoading]);
  ...
  const handleDeleteArticleCategory = async (id: number) => {
    try {
      await apiClient.delete(`/article-categories/${id}`);

      mutate();
      toast.success("Berhasil menghapus kategori artikel");
    } catch (error) {
      console.log(error);
      toast.error("Gagal menghapus kategori artikel");
    }
  };

  if (isLoading) {
    return toast.loading("Memuat data...", { id: "article-categories" });
  } else {
    toast.dismiss("article-categories");
  }
  ...
  return (
    <>
      <div className="flex flex-wrap gap-4 justify-between items-center mb-4">
        ...
        <div className="flex gap-4 items-center justify-end flex-wrap">
          <Button
            ...
            asChild
          >
            <Link href="/dashboard/article-categories/new">
              <Plus />
              Buat Data
            </Link>
          </Button>
      ...
      <Table className="min-w-[1200px] shadow-md relative bg-white">
        ...
        <TableBody>
          {!isLoading && articleCategories!.length === 0 && !searchTerm ? (
            <TableRow className="text-sm">
              <TableCell colSpan={12} className="text-center h-24">
                Tidak ada data kategori artikel.
              </TableCell>
            </TableRow>
          ) : !isLoading && articleCategories!.length === 0 && searchTerm ? (
            <TableRow className="text-sm">
            ...
            articleCategories!.map((category) => (
              <TableRow key={category.id} className={`hover:bg-gray-50 text-sm`}>
                <TableCell>
                  <DropdownMenu
                    open={openMenuId === category.id}
                    onOpenChange={(open) =>
                      open ? setOpenMenuId(category.id) : setOpenMenuId(null)
                    }
                  >
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem
                        ...
                        onClick={() =>
                          router.push(`/dashboard/article-categories/${category.id}/edit`)
                        }
                      >
                        Edit
                      </DropdownMenuItem>
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                              onClick={() => handleDeleteArticleCategory(category.id)}
